declare var CELLS: any;
declare var BACKPACK: any;
declare var BACKPACK_SIZE: any;
declare var GRID_SIZE: any;

(() => {
    //The SVG icons
    const robot_svg = '<svg class="robot" style="width: 100%;height: 100%" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512"><path d="M320 0c17.7 0 32 14.3 32 32V96H472c39.8 0 72 32.2 72 72V440c0 39.8-32.2 72-72 72H168c-39.8 0-72-32.2-72-72V168c0-39.8 32.2-72 72-72H288V32c0-17.7 14.3-32 32-32zM208 384c-8.8 0-16 7.2-16 16s7.2 16 16 16h32c8.8 0 16-7.2 16-16s-7.2-16-16-16H208zm96 0c-8.8 0-16 7.2-16 16s7.2 16 16 16h32c8.8 0 16-7.2 16-16s-7.2-16-16-16H304zm96 0c-8.8 0-16 7.2-16 16s7.2 16 16 16h32c8.8 0 16-7.2 16-16s-7.2-16-16-16H400zM264 256a40 40 0 1 0 -80 0 40 40 0 1 0 80 0zm152 40a40 40 0 1 0 0-80 40 40 0 1 0 0 80zM48 224H64V416H48c-26.5 0-48-21.5-48-48V272c0-26.5 21.5-48 48-48zm544 0c26.5 0 48 21.5 48 48v96c0 26.5-21.5 48-48 48H576V224h16z"/></svg>';
    ...
    //-------------------------------------------------------
    //Energy bar
    let bar = new ldBar(".ldBar", {}); //Energy bar
    //To change dynamically the size of the grid and the backpack
    let updateDims = () => {
        let grid_container = document.getElementById('grid_container');
        let backpack_container = document.getElementById('backpack_container');
        let grid_style = window.getComputedStyle(grid_container);
        let backpack_style = window.getComputedStyle(backpack_container);

        let grid_width = window.innerHeight - parseInt(grid_style.marginTop, 10) - parseInt(grid_style.marginBottom, 10);
        let backpack_width = ((window.innerWidth - grid_width) / 2) - parseInt(backpack_style.marginLeft, 10) - parseInt(backpack_style.marginRight, 10);
        let max_backpack_width = ((grid_width * 1.95) / BACKPACK_SIZE);
    ...
    const ws_open = () => {
        ...
        ws.onmessage = (e) => {
            ...
            let energy = jsonResponse.energy ?? 0;
            let message = jsonResponse.test_string ?? ''
            let world = jsonResponse.world ?? [];
            let backpack = jsonResponse.backpack ?? {};
            let ui_coordinate = jsonResponse.ui_coordinate ?? [0, 0];
            let grid_coordinate = jsonResponse.grid_coordinate ?? [0, 0];

            //Aggiorno l'enegia
            bar.set(energy);

            //Aggiorno le coordinate
            let p_coordinate = document.getElementById('p-coordinate');
            p_coordinate.innerText = `[${grid_coordinate[0]}, ${grid_coordinate[1]}] -> [${ui_coordinate[0]}, ${ui_coordinate[1]}] ${message}`;

            //Aggioro lo zaino
            for (let item in backpack) {
                if (item.toLowerCase() !== 'none') {
                    let item_count_div = document.getElementById(`item-count-${item}`);
            ...
            let new_cells = new Set(CELLS);

            //Per ogni cella occupata nel mondo
                for (let i = 0; i < world.length; i++) {
                    let id = world[i].id;
                    let tile_type = world[i].tile_type;
                    let elevation = world[i].elevation;
                    let content = world[i].content;
                    ...
                    if (content != null && content !== 'None')
                    {
                        ...
                        {
                            let content_type = Object.keys(content)[0];
                            let content_numer = content[content_type];
                ...
                //Mostro il robot
                let robot_cell = document.getElementById(`cell-${ui_coordinate[0] * GRID_SIZE +ui_coordinate[1]}`);
            ...
            const fillBackpack = () => {
                ...
                BACKPACK.forEach((item) => {
                    let item_div = document.getElementById("item-"+item+"");
                    let item_count_div = document.getElementById("item-count-" + item + "");
            ...
            const selectBackground = (tile_type) => {
                let background = '';
                switch (tile_type) {
                    case 'DeepWater':
                        background = 'DarkBlue';
